...
/**
 * Custom OAuth Access token holder for exact API.
 */
public class ExactDefaultOAuth2AccessToken implements Serializable,
		OAuth2AccessToken {
	 ...
	 */
	public ExactDefaultOAuth2AccessToken(String value) {
		this.value = value;
	}
	...
	private ExactDefaultOAuth2AccessToken() {
		this((String) null);
	}
	 ...
	 * @param accessToken
	 */
	public ExactDefaultOAuth2AccessToken(OAuth2AccessToken accessToken) {
	 ...
	 * doesn't define (yet) that the valid token types are, but says it's
	...
	public static OAuth2AccessToken valueOf(Map<String, String> tokenParams) {
		ExactDefaultOAuth2AccessToken token = new ExactDefaultOAuth2AccessToken(
				tokenParams.get(ACCESS_TOKEN));

		if (tokenParams.containsKey(EXPIRES_IN)) {
			long expiration = 0;
			try {
				expiration = Long.parseLong(String.valueOf(tokenParams
						.get(EXPIRES_IN)));
		...
		if (tokenParams.containsKey(SCOPE)) {
			...
			for (StringTokenizer tokenizer = new StringTokenizer(
					tokenParams.get(SCOPE), " ,"); tokenizer.hasMoreTokens();) {
	...
	/**
	 * Additional information that token granters would like to add to the
	 * token, e.g. to support new token types. If the values in the map are
	 * primitive then remote communication is going to always work. It should
	 * also be safe to use maps (nested if desired), or something that is
